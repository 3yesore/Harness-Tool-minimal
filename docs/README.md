# 文档中心

这里收纳的是辅助说明、发布材料和版本展示内容。仓库主页默认展示 `v1.0.1 beta`，`v1.0.0` 保留为历史版本锚点。

## 优先阅读

- [README.md](../README.md)
- [INDEX.md](INDEX.md)
- [HARNESS_SPEC.md](HARNESS_SPEC.md)
- [GITHUB_RELEASE.md](GITHUB_RELEASE.md)
- [VERSION_ROADMAP.md](VERSION_ROADMAP.md)

## 版本与展示

- `README.md`：仓库默认主页，当前展示 `v1.0.1 beta`
- `README.en.md`：英文镜像主页
- `docs/VERSION_ROADMAP.md`：当前版本与路线图
- `docs/GITHUB_RELEASE.md`：GitHub 发布页说明
- `docs/RELEASE_NOTES_v1.0.md`：历史版本 `v1.0.0` 的说明

## 辅助文档

- [AI_CHECKLIST.md](AI_CHECKLIST.md)
- [AI_REPAIR_GUIDE.md](AI_REPAIR_GUIDE.md)
- [CONTRIBUTING.md](CONTRIBUTING.md)
- [DESIGN_REVIEW.md](DESIGN_REVIEW.md)
- [FAQ.md](FAQ.md)
- [GITHUB_PUBLISH_GUIDE.md](GITHUB_PUBLISH_GUIDE.md)
- [RELEASE_PLAYBOOK.md](RELEASE_PLAYBOOK.md)
- [AI_OPERATIONS.md](AI_OPERATIONS.md)
- [EXTENSION_POINTS.md](EXTENSION_POINTS.md)

## 说明

- 中文主入口在根目录 `README.md`
- 英文主入口在根目录 `README.en.md`
- 发布前先看根目录首页，再根据需要进入这里的专题文档
- 每次发布新版本，优先看 `RELEASE_PLAYBOOK.md`
