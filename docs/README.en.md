# Documentation Hub

This folder collects supporting docs, release materials, and version display pages. The repository homepage defaults to `v1.0.1 beta`, while `v1.0.0` is kept as the historical anchor.

## Start Here

- [README.en.md](../README.en.md)
- [INDEX.en.md](INDEX.en.md)
- [HARNESS_SPEC.en.md](HARNESS_SPEC.en.md)
- [GITHUB_RELEASE.en.md](GITHUB_RELEASE.en.md)
- [VERSION_ROADMAP.en.md](VERSION_ROADMAP.en.md)

## Versions and Displays

- `README.en.md`: repository homepage, currently showing `v1.0.1 beta`
- `README.md`: Chinese homepage mirror
- `docs/VERSION_ROADMAP.en.md`: current version and roadmap
- `docs/GITHUB_RELEASE.en.md`: GitHub release page notes
- `docs/RELEASE_NOTES_v1.0.md`: historical notes for `v1.0.0`

## Supporting Docs

- [AI_CHECKLIST.md](AI_CHECKLIST.md)
- [AI_REPAIR_GUIDE.md](AI_REPAIR_GUIDE.md)
- [CONTRIBUTING.md](CONTRIBUTING.md)
- [DESIGN_REVIEW.md](DESIGN_REVIEW.md)
- [FAQ.md](FAQ.md)
- [GITHUB_PUBLISH_GUIDE.md](GITHUB_PUBLISH_GUIDE.md)
- [RELEASE_PLAYBOOK.en.md](RELEASE_PLAYBOOK.en.md)
- [AI_OPERATIONS.en.md](AI_OPERATIONS.en.md)
- [EXTENSION_POINTS.en.md](EXTENSION_POINTS.en.md)

## Notes

- The Chinese homepage is `README.md`
- The English homepage is `README.en.md`
- Read the root homepage first, then use this hub for topic-specific materials
- For recurring releases, start with `RELEASE_PLAYBOOK.en.md`
